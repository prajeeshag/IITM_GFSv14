...
if [ $machine = WCOSS_C ]; then
  . $MODULESHOME/init/sh
####  echo module load PrgEnv-intel intel cray-mpich
####  echo module load prod_envir prod_util grib_util
...
export LANDICE_OPT=${LANDICE_OPT:-5}    #Initially 2
...
cat <<EOFA > chgres_submit.txt
 $APRUNC $CHGRESEXEC $REDOUT$PGMOUT $REDERR$PGMERR
