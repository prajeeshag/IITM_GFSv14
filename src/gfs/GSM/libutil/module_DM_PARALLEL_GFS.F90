...
!
      integer(kind=kind_io4),allocatable,dimension(:) :: irank, num_serv_per_grp
!
...
!
!!!   mype=mype_share
      my_taskid = mype                    !set this for use by print timers
...
!
      ...
      if(mype==0) write(0,*)'my color=',icolor
!
!-----------------------------------------------------------------------
...
!
      ...
      if (mype == 0) write(0,*)'after mpi_comm_split'
!
!-----------------------------------------------------------------------
...
!!!     iworld=mpi_comm_world
        ...
        if (mype == 0) write(0,*)'after mpi_comm_create'
!
!-----------------------------------------------------------------------
...
!         write(0,*)'before mpi_intercomm_create'
          call mpi_intercomm_create(mc_comp                             & !<-- Each task's local intracommunicator
                                   ...
                                   ,ierr)
!         write(0,*)'after mpi_intercomm_create,mpi_comm(',i,')=', mpi_comm_inter_array(i)
...
!***
      npes = npes - iqserver
      if (mype == 0) write(0,*)'npes=',npes,'last_fcst_pe=',last_fcst_pe
!
      if(mype==0)then
