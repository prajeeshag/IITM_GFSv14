...
!***                           and some cleanup
!***  Apr 12 2016   S Moorthi  changed semilag to pdp_output
!***  Apr 18 2016   s Moorthi  Removing the bug related to ntke added by Jun during
!***                           the last trunk update
!***  May 04 2016   S Moorthi  add ntiw, ntlnc, ntinc nto and nto2
!***
!-----------------------------------------------------------------------
!
      USE ESMF
      ...
      use namelist_dynamics_def,          only: pdp_output
...
!
      ...
      I_SC(15)%NAME => itrun
      I_SC(16)%NAME => icen2
      I_SC(17)%NAME => int_state%ntiw
      I_SC(18)%NAME => int_state%ntlnc
      I_SC(19)%NAME => int_state%ntinc
      I_SC(20)%NAME => int_state%nto
      I_SC(21)%NAME => int_state%nto2
!        
!***  REAL SCALARS
!     ------------
!
...
!-----------------------------------------------------------------------
!
      CALL ADD_BUNDLE_TO_WRITE(int_state, imp_state_write, pdp_output,  &
...
!-----------------------------------------------------------------------
!
      SUBROUTINE ADD_BUNDLE_TO_WRITE(int_state, imp_state_write, pdp_output,&
                                     GRID, I_SC, R_SC, L_SC, I_1D,          &
                                     R_1D, R_2D, R_3D,                      &
...
!  ntke                       ! this is NOT needed - ntrac above already includes tke
...
!
        IF(trim(DYN_INT_STATE_3D_R(2,NFIND)) == 'OGFS_SIG') THEN            !<-- Take 3D real array data specified for history output
          ...
          if (.not. pdp_output .and. (trim(DYN_INT_STATE_3D_R(1,NFIND)) == 'dpres' &
...
!
          DO K=1,NDIM3
            ...
            VBL_NAME = TRIM(DYN_INT_STATE_3D_R(1,NFIND))//'__'//MODEL_LEVEL//'_2D'
...
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
            if (mype == 0) CALL gfs_dynamics_err_msg(RC,MESSAGE_CHECK,RC_DYN_OUT)
!           if (mype == 0 .and. rc_dyn_out /= 0) write(0,*)' RC=',rc,' for VBL_NAME=',VBL_NAME
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
!
! ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~ ~
