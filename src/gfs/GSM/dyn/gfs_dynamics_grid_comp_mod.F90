...
!
      ...
      kdt_start            = advancecount4
       ...
       if (int_state%me == 0) write(0,*)'in dyn_grid_comp,advancecount4=',&
          advancecount4,' kdt_start=',kdt_start, 'phour=',int_state%phour,&
          'kfhour=',int_state%kfhour,'kdt=', int_state%kdt
!
...
!------------------------------------------------------------
      ...
      tend=timef()
      dynamics_timer(1)%elapsed = dynamics_timer(1)%elapsed         &
     &                           + (tend -tbeg)
...
!
      IF(.NOT. int_state%restart_step .AND. .NOT. int_state%start_step ) THEN
        IF(.NOT. int_state%reset_step) THEN
         ...
         tend=timef()
         dynamics_timer(3)%elapsed = dynamics_timer(3)%elapsed           &
     &                              + (tend -tbeg)
        ELSE
...
! ======================================================================
      ...
      tend=timef()
      dynamics_timer(4)%elapsed = dynamics_timer(4)%elapsed           &
     &                              + (tend -tbeg)
...
!---------------------------------------------------------------
      ...
      tend=timef()
      dynamics_timer(2)%elapsed = dynamics_timer(2)%elapsed         &
     &                           + (tend -tbeg1)
...
!---------------------------------------------------------------
      ...
      tend=timef()
      dynamics_timer(7)%elapsed = dynamics_timer(7)%elapsed         &
     &                              + (tend -tbeg)
