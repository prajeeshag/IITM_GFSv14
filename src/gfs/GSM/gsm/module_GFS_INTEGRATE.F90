...
!-----------------------------------------------------------------------
      ...
      use module_timers,only: general_timer,timef
!
!-----------------------------------------------------------------------
!
      IMPLICIT NONE
...
!
        ...
        CALL ESMF_GridCompRun(gridcomp    = GC_GFS_DYN                  &
                             ...
                             ,rc          = RC)
        tend=timef()
        general_timer(5)%elapsed = general_timer(5)%elapsed         &
     &                            + (tend -tbeg)
!
...
!         IF(LALM1 .or. LALM2) THEN
        ...
        if (mype == 0) print *,' NTIMESTEP=',NTIMESTEP,' aft disable alarm'  &
                              ,' NTIMESTEPB=',NTIMESTEPB,'DFIHR=',DFIHR
              else
...
!             if (mype == 0) write(0,*)' setting clock back after digital filter'
              ...
              donetime  = currtime - starttime
              NTIMESTEP = nint(donetime/timeStep)
              if (mype == 0) write(0,*)' after setting clock back NTIMESTEP=',NTIMESTEP
...
!
            ...
            END IF
        tend=timef()
        general_timer(6)%elapsed = general_timer(6)%elapsed         &
     &                            + (tend -tbeg)
          ENDIF  filter_block
        END IF
...
!
        ...
        if (mype == 0) print *,' NTIMESTEP=',NTIMESTEP,' aft enable alarm'  &
                              ,' NTIMESTEPB=',NTIMESTEPB,'DFIHR=',DFIHR
            else
...
!
        LSPC = (NTIMESTEP == 1 .and. currtime-timestep == starttime) .OR. &
               (LDFIFLTO .and. CurrTime == halfdfitime .and. DFIHR==0     &
               .and. .not.LSKIP)
...
!
          ...
          call esmf_cplcomprun(cplcomp     = gc_gfs_cpl          &
                              ...
                              ,rc          = RC)
        tend=timef()
        general_timer(7)%elapsed = general_timer(7)%elapsed         &
     &                            + (tend -tbeg)
...
!
            ...
            call esmf_gridcomprun(gridcomp    = gc_gfs_phy            &
                                 ...
                                 ,rc          = RC)
        tend=timef()
        general_timer(8)%elapsed = general_timer(8)%elapsed         &
     &                            + (tend -tbeg)
...
!
         tbeg=timef()
          call esmf_cplcomprun(cplcomp     = gc_gfs_cpl             &
                              ...
                              ,rc          = RC)
        tend=timef()
        general_timer(9)%elapsed = general_timer(9)%elapsed         &
     &                            + (tend -tbeg)
!
...
!         -----------------------------------------------
      ...
      tend=timef()
      general_timer(4)%elapsed = general_timer(4)%elapsed         &
     &                           + (tend -tbeg1)
