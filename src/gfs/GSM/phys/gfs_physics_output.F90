...
!***  Jul    2016  J. Wang   Add isot and ialb in gfs flx file
!
!-----------------------------------------------------------------------
!
      USE ESMF
...
!
       =RESHAPE((/ 'uflx_ave        ', 'OGFS_FLX        ', 'sfc             ' &   !1
                  ...
                  ,'tmax_max        ', 'OGFS_FLX        ', '2 m above gnd   ' &   !55
                  ,'tmin_min        ', 'OGFS_FLX        ', '2 m above gnd   ' &   !56
                  ,'spfhmax_max     ', 'OGFS_FLX        ', '2 m above gnd   ' &   !57
                  ,'spfhmin_min     ', 'OGFS_FLX        ', '2 m above gnd   ' &   !58
...
!
       ...
       use resol_def,            only: thermodyn_id,sfcpress_id,ivssfc,ivsnst
...
!                                      ngrids_gg,ivssfc,ivsnst
       ...
       use namelist_physics_def, only: nstf_name,ivegsrc,cu_physics,ialb,   &
                                       isot
!
...
!
      DO NFIND=1,MAX_KOUNT
        IF(trim(PHY_INT_STATE_2D_R(file_index,NFIND)) == trim(file_id)) THEN    !<-- Take 2D real array data specified for history output
          VBL_NAME = TRIM(PHY_INT_STATE_2D_R(1,NFIND))//'__'//trim(PHY_INT_STATE_2D_R(3,NFIND))
          if(file_id == 'OGFS_SFC') then
...
!
            VBL_NAME = TRIM(PHY_INT_STATE_3D_R(1,NFIND))//'__'//TRIM(PHY_INT_STATE_3D_R(3,NFIND)) &
     &         //'__'//MODEL_LEVEL //'_2D'
            NULLIFY(TEMP_R2D)
...
!
         DO N=1,NDIM4                                                  !<-- Loop through the tracers kind
          DO K=1,NDIM3                                                 !<-- Loop through the levels of the array
            ...
            WRITE(MODEL_LEVEL, FMT) K
!
              VBL_NAME = TRIM(PHY_INT_STATE_4D_R(1,NFIND))//'__'//TRACERS_KIND//'__'//MODEL_LEVEL//'_2D'
              NULLIFY(TEMP_R2D)
