                 ...
                 int f, unsigned char verbose, unsigned char missing)
  {
   ...
   for (v=0; v < ncinfile->nvars; v++)
       ...
       else
	 {
	   ...
	   iioffset=0; jjoffset=0; kkoffset=0; lloffset=0;
         switch (ncinfile->datatype[v])
           {
            case NC_BYTE:
            case NC_CHAR:
              ...
              b=0;
              for (l=0; l < lmax; l++) {
		if (loffset > 0) lloffset=ncinfile->decomp[l+1]-1;
		else lloffset=l;
                for (k=0; k < kmax; k++) {
		  if (koffset > 0) kkoffset=ncinfile->decomp[k+1]-1;
		  else kkoffset=k;
                  for (j=0; j < jmax; j++) {
		    if (joffset > 0) jjoffset=ncinfile->decomp[j+1]-1;
		    else jjoffset=j;
                    for (i=0; i < imax; i++) {
		      if (ioffset > 0) iioffset=ncinfile->decomp[i+1]-1;
		      else iioffset=i;
