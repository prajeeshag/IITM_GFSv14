/*
  ...
  GFS version: Prajeesh A G
               modified to combine the gfs output files.
               cannot be used for mom files.
...
int nccp2r(int argc, char *argv[])
{
  ...
  for (a = 1; a < argc; a++)
  {
    ...
    else if (!strcmp(argv[a], "-h"))
    {
      ...
      missing = 1;
    //else if (!strcmp(argv[a],"-c")) compress=1;
    else
    {
      outputarg = a;
      break;
  ...
  {
    if (bf != 1)
      fprintf(stderr, "-x is set, so blocking factor will be set to 1. The -k option will be ignored.\n");
    bf = 1;
    if (verbose)
      printf("This run will estimate peak memory resident size. No output file will be created.\n");
  }
    ...
    if (nend > -1)
      for (block = 0; block < nblocks; block++)
      {
        ...
        for (a = nstart; a <= nend; a++)
        {
          ...
          if (verbose)
          {
            if (block == 0)
              ...
              printf("  %d files to go... ", num_infiles_used - f);
            printf("processing \"%s\"\n", infilename);
          }
        ...
        if (verbose && print_mem_usage)
          printf("freeing memory allocated in block\n");
        for (k = 0; k < bf; k++)
                 ...
                 unsigned char verbose, unsigned char missing)
{
  ...
  nc_type decomp_type; /* attribute types */
  int decomp_len;
  ...
  *nfiles = nfiles2 - 1;
  ...
  if (ncattinq(ncinfile->ncfid, NC_GLOBAL, "decomp", &decomp_type, &decomp_len) == (-1))
  {
    fprintf(stderr, "Error: cannot find decomp global attribute!\n");
    ncclose(ncinfile->ncfid);
    free(ncinfile);
    return (1);
  }
  ncinfile->decomp = (int *)malloc(decomp_len * nctypelen(decomp_type));

  if (ncattget(ncinfile->ncfid, NC_GLOBAL, "decomp", (void *)ncinfile->decomp) == (-1))
  {
    fprintf(stderr, "Error: error while reading decomp global attribute!\n");
    ncclose(ncinfile->ncfid);
    free(ncinfile);
    return (1);
  }
  ...
  for (d = 0; d < ncinfile->ndims; d++)
  {
    ...
    ncinfile->dimend[d] = (-1);
    if (!strcmp(ncinfile->dimname[d], "lat"))
    {
      ncinfile->dimfullsize[d] = ncinfile->decomp[0];
      ncinfile->dimstart[d] = ncinfile->decomp[1];
    }
  }
  ...
  for (v = 0; v < ncinfile->nvars; v++)
  {
    ...
    /* if ((dimid=ncdimid(ncinfile->ncfid,"lat"))!=(-1)) */
    /*   { */
  ...
  for (v = 0; v < ncinfile->nvars; v++)
  {
    ...
    for (d = 0; d < ncinfile->varndims[v]; d++)
    {
      ...
      if (!strcmp(ncinfile->dimname[ncinfile->vardim[v][d]], "lat"))
      {
  ...
  if ((!appendnc) && (!mem_dry_run))
  {
    ...
    for (v = 0; v < ncinfile->nvars; v++)
    {
      ...
      /* if (compress == 1)
           {
	     if ((retval=nc_def_var_deflate(ncoutfile->ncfid,v,1,1,3))) ERR(retval);
	   } */
      for (n = 0; n < ncinfile->natts[v]; n++)
      {
        ...
        if (!strcmp(attname, "domain_decomposition"))
          continue;
        else
        {
  ...
  do
  {
    ...
    if (verbose && print_mem_usage)
      if (mem_for_rec > 0)
        printf("  mem alloc for r=%d, infile=%s is %lu KB\n", r, ncname, mem_for_rec / 1024);
    r++;
    appendnc = 1;
                 ...
                 int f, unsigned char verbose, unsigned char missing)
{
  ...
  int iioffset, jjoffset, kkoffset, lloffset;
    ...
    *nrecs = recdimsize;
    ...
    if ((*bf) >= 1)
    {
      if ((*bf) > (*nrecs))
      {
        if ((*nrecs) <= 0)
        {
          fprintf(stderr, "Error: nrecs equals to zero. \n");
          return (1);
        }
    ...
    if (verbose)
      fprintf(stderr, "blocking factor=%d, num. blocks=%d, num. records=%d\n", *bf, *nblocks, *nrecs);
  }
  ...
  if (varbuf == NULL)
  {
    ...
    if (verbose || print_mem_usage)
      fprintf(stderr, "allocating a buffer of %d bytes for the multi-dimensional pointer array\n", nbytes);
    if (mem_dry_run)
      estimated_maxrss += nbytes;
  ...
  for (v = 0; v < ncinfile->nvars; v++)
  {
    ...
    else
    {
      ...
      if (ncinfile->varndims[v] > 1)
      {
        ...
        if (dimid == ncinfile->recdim)
          jmax = 1;
        else
          jmax = ncinfile->dimsize[dimid];
      }
      else
        jmax = 1;
      ...
      case NC_CHAR:
        if (verbose > 1)
          printf("      start copying byte/char\n");
        b = 0;
        for (l = 0; l < lmax; l++)
        {
          if (loffset > 0)
            lloffset = ncinfile->decomp[l + 1] - 1;
          else
            lloffset = l;
          for (k = 0; k < kmax; k++)
          {
            if (koffset > 0)
              kkoffset = ncinfile->decomp[k + 1] - 1;
            else
              kkoffset = k;
            for (j = 0; j < jmax; j++)
            {
              if (joffset > 0)
                jjoffset = ncinfile->decomp[j + 1] - 1;
              else
                jjoffset = j;
              for (i = 0; i < imax; i++)
              {
                if (ioffset > 0)
                  iioffset = ncinfile->decomp[i + 1] - 1;
                else
                  iioffset = i;

                offset = (iioffset) +
                         (jjoffset)*imaxfull +
                         (kkoffset)*imaxjmaxfull +
                         (lloffset)*imaxjmaxkmaxfull;
        ...
        if (verbose > 1)
          printf("      end copying byte/char\n");
        break;
      case NC_SHORT:
        if (verbose > 1)
          printf("      start copying short\n");
        b = 0;
        for (l = 0; l < lmax; l++)
        {
          if (loffset > 0)
        ...
        if (verbose > 1)
          printf("      end copying short\n");
        break;
      case NC_INT:
        if (verbose > 1)
          printf("      start copying int\n");
        b = 0;
        for (l = 0; l < lmax; l++)
        {
          if (loffset > 0)
        ...
        if (verbose > 1)
          printf("      end copying int\n");
        break;
      case NC_FLOAT:
        if (verbose > 1)
          printf("      start copying float\n");
        b = 0;
        for (l = 0; l < lmax; l++)
        {
          if (loffset > 0)
        ...
        if (verbose > 1)
          printf("      end copying float\n");
        break;
      case NC_DOUBLE:
        if (verbose > 1)
          printf("      start copying double\n");
        b = 0;
        for (l = 0; l < lmax; l++)
        {
          if (loffset > 0)
