...
# run post to read nemsio file if OUTTYP=4
 ...
 export nemsioget=~/nemsio/bin/nemsio_get
 export LONB=`aprun -n 1 $nemsioget $NEMSINP lonf |grep -i "lonf" |awk -F"= " '{print $2}' |awk -F" " '{print $1}'`
 export LATB=`aprun -n 1 $nemsioget $NEMSINP latg |grep -i "latg" |awk -F"= " '{print $2}' |awk -F" " '{print $1}'`
 export JCAP=`aprun -n 1 $nemsioget $NEMSINP jcap |grep -i "jcap" |awk -F"= " '{print $2}' |awk -F" " '{print $1}'`
