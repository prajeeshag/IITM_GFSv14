...
# commented by MSDRSD on 11 Jan 2016
# Set the PDY var if not already set
#if test -z "$PDY"
#then
#   cp $basesource/com/date/$cycle ncepdate
#   cp $basesource/com/date/$cycle NMCDATE
#   PDY=`cut -c7-14 $basesource/date/$cycle`
#else
#   PDYbad=`cut -c7-14 $basesource/com/date/$cycle`
#   sed s/$PDYbad/$PDY/g $basesource/com/date/$cycle > ncepdate
#   cp ncepdate NMCDATE
#fi
# commented by MSDRSD on 11 Jan 2016
...
export NWPROD1=/scratch/cccr/prajeesh/GFSv14_bin
DATElne_minus=`sh $NWPROD1/finddate.sh $PDY s-10`
DATElne_plus=`sh $NWPROD1/finddate.sh $PDY s+10`
