...
!   use wti(:,:,1) for x-derivative, wti(:,:,2) for y-derivative, wti(:,:,3) for xy-derivative  
    ...
    if ( verbose_bicubic > 0 ) then
       write (stdout(),'(/,"Initialising bicubic interpolation, interface horiz_interp_bicubic_init_1d_s")')
       write (stdout(),'(/," Longitude of coarse grid points (radian): xc(i) i=1, ",i4)') Interp%nlon_src
       write (stdout(),'(1x,10f10.4)') (Interp%lon_in(jj),jj=1,Interp%nlon_src)
       write (stdout(),'(/," Latitude of coarse grid points (radian):  yc(j) j=1, ",i4)') Interp%nlat_src
       write (stdout(),'(1x,10f10.4)') (Interp%lat_in(jj),jj=1,Interp%nlat_src)
       do i=1, Interp%nlat_dst
         write (stdout(),*)
         write (stdout(),'(/," Longitude of fine grid points (radian): xf(i) i=1, ",i4)') Interp%nlat_dst
         write (stdout(),'(1x,10f10.4)') (lon_out(jj,i),jj=1,Interp%nlon_dst)
       enddo
       do i=1, Interp%nlon_dst 
         write (stdout(),*)
         write (stdout(),'(/," Latitude of fine grid points (radian):  yf(j) j=1, ",i4)') Interp%nlon_dst
         write (stdout(),'(1x,10f10.4)') (lat_out(i,jj),jj=1,Interp%nlat_dst)
       enddo
    endif  
...
!     the source grid
  ...
  subroutine horiz_interp_bicubic_init_1d ( Interp, lon_in, lat_in, lon_out, lat_out, &
    ...
    if ( verbose_bicubic > 0 ) then
       write (stdout(),'(/,"Initialising bicubic interpolation, interface horiz_interp_bicubic_init_1d")')
       write (stdout(),'(/," Longitude of coarse grid points (radian): xc(i) i=1, ",i4)') Interp%nlon_src
       write (stdout(),'(1x,10f10.4)') (Interp%lon_in(jj),jj=1,Interp%nlon_src)
       write (stdout(),'(/," Latitude of coarse grid points (radian):  yc(j) j=1, ",i4)') Interp%nlat_src
       write (stdout(),'(1x,10f10.4)') (Interp%lat_in(jj),jj=1,Interp%nlat_src)
       write (stdout(),*)
       write (stdout(),'(/," Longitude of fine grid points (radian): xf(i) i=1, ",i4)') Interp%nlat_dst
       write (stdout(),'(1x,10f10.4)') (lon_out(jj),jj=1,Interp%nlon_dst)
       write (stdout(),'(/," Latitude of fine grid points (radian):  yf(j) j=1, ",i4)') Interp%nlon_dst
       write (stdout(),'(1x,10f10.4)') (lat_out(jj),jj=1,Interp%nlat_dst)
    endif  
