...
type data_type
   ...
   character(len=128) :: fieldname_file ! fieldname used in the netcdf data file
   character(len=128) :: file_name   ! name of netCDF data file
   integer            :: interpol_method   ! interpolation method (default BILINEAR
   real :: factor ! For unit conversion, default=1, see OVERVIEW above
end type data_type
...
type override_type
 ...
 integer, parameter :: NONE     = 0 ! on grid, no horizontal interpolation
 integer, parameter :: BILINEAR = 1 ! bilinear horizontal interpolation
 integer, parameter :: BICUBIC  = 2 ! bicubic horizontal interpolation
...
! </NOTE>
  integer :: is,ie,js,je
  integer :: i, iunit, io_status, ntable, ntable_old, ntable_new
  character(len=256) :: record
  type(data_type_new) :: data_entry_new
  type(data_type_old) :: data_entry_old
...
!3 Read coupler_table 
    ...
    do while (ntable <= max_table)
       ...
       if (index(lowercase(record), ".false.") .ne. 0 .or. index(lowercase(record), ".true.") .ne. 0 ) then   ! old format
          ntable_old = ntable_old + 1
          read(record,*,err=99) data_entry_old
          data_entry%gridname       = data_entry_old%gridname
          data_entry%fieldname_code = data_entry_old%fieldname_code
          data_entry%fieldname_file = data_entry_old%fieldname_file
          data_entry%file_name      = data_entry_old%file_name
          data_entry%factor         = data_entry_old%factor 
          if(data_entry_old%ongrid) then
          ...
          read(record,*,err=99) data_entry_new
          data_entry%gridname       = data_entry_new%gridname
          data_entry%fieldname_code = data_entry_new%fieldname_code
          data_entry%fieldname_file = data_entry_new%fieldname_file
          data_entry%file_name      = data_entry_new%file_name
          data_entry%factor         = data_entry_new%factor 
          select case (trim(lowercase(data_entry_new%interpol_method)))
          case ("none")
             data_entry%interpol_method = NONE
          case ("bilinear", "default")
             data_entry%interpol_method = BILINEAR
          case ("bicubic")
             data_entry%interpol_method = BICUBIC
          case default
             write(stdout(),*)" gridname is ", trim(data_entry_new%gridname)
             write(stdout(),*)" fieldname_code is ", trim(data_entry_new%fieldname_code)
             write(stdout(),*)" fieldname_file is ", trim(data_entry_new%fieldname_file)
             write(stdout(),*)" file_name is ", trim(data_entry_new%file_name)
             write(stdout(),*)" factor is ", data_entry_new%factor 
             write(stdout(),*)" interpol_method is ", trim(data_entry_new%interpol_method)
...
100 continue
    table_size = ntable
    if (ntable_new*ntable_old .ne. 0) call mpp_error(FATAL, &
         'data_override_mod: do not support both format data entry, check your data_override_table')
