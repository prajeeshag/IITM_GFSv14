...
!
  ...
  use gfs_comm_mod, only: init_local_comm, set_gfs_exchange_communicator, end_gfs_comm !Prajeesh-> Gfs coupling
...
!#######################################################################
  ...
  !->prajeesh GFS coupling
  call init_local_comm('OCN',localcomm)
...
!-----------------------------------------------------------------------
  ...
  call fms_end
  call end_gfs_comm() !-> Prajeesh, GFS coupling
!-----------------------------------------------------------------------
...
!    possible error condition needs to be checked
    ...
    if( Atm%pe )then
        ...
        call set_gfs_exchange_communicator() !-> Prajeesh, GFScoupling
    elseif( Ocean%pe )then  ! Error check above for disjoint pelists should catch any problem
        ...
        call set_gfs_exchange_communicator(-1) !->Prajeesh, GFScoupling
    end if
