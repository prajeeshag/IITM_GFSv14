...
!-----------------------------------------------------------------------
 ...
 real, private                    :: sea_ice_threshld_output
...
!-----------------------------------------------------------------------
 ...
 if (kgds_output(1) == 4 .or. kgds_output(1) == 0) then ! gaussian/latlon grid
   ...
   call get_ext_climo_nmm(imdl_output,jmdl_output,hour, & 
                          month, day, year, iret)
...
!-----------------------------------------------------------------------
 ...
 if (mdl_res_input < (0.35*mdl_res_output)) then
   ...
   ipopt(20) = nint(1.0 / mdl_res_input) + 1   ! search box width of 1 deg.
...
!-----------------------------------------------------------------------
 ...
 if (.not. allocated (input%sea_ice_fract)) then  !input grid is pre-seaice model
   ...
   do ij = 1, count_nonland_output
     if (bitmap_nonland_output(ij)) then
       if (output_data_nonland(ij) >= sea_ice_threshld_output) then
...
!$$$
 ...
 namelist /options/                climo_fields_opt, &
                                   landice_opt,      &
                                   sea_ice_threshld_output

 sea_ice_threshld_output = 0.15
