...
!     INTEGER ZAVG(IMN,JMN),ZSLM(IMN,JMN)
      ...
      DIMENSION IWORK(IM,JM,4),WORK1(IM,JM),
     1     WORK2(IM,JM),WORK3(IM,JM),WORK4(IM,JM),
     2     WORK5(IM,JM),WORK6(IM,JM),
     3     GLAT(JMN)
      LOGICAL SPECTR, REVLAT, FILTER
       logical*1 lb(imn*jmn)
      DIMENSION HPRIME(IM,JM,14)
...
!     MSKOCN = 1             ! Ocean land sea mask =1, =0 if not present
      MSKOCN = 0             ! Ocean land sea mask =1, =0 if not present
...
! --- If one wants the slm without ocean model dictates, then set MSKOCN=0
...
!       open(10,
!     &'/home/SSPMRES/rphani/test/mtnvar_to_nc/terr_lsm/
!     &landcover30.fixed',
!    &'/ptmpd2/Jordan.Alpert/terr/landcover30.fixed',
       open(10,form='unformatted',
!     &  access='direct',recl=43200*21600,iostat=ist1)
     &  access='direct',recl=21600*10800,iostat=ist1)
!     & recl=43200*21600, access='direct',iostat=istat)

                  IF (istat.ne.0) then
                   MSKSRC=-1
      print *,' UMD lake mask open failed -- istat,MSKSRC:',istat,MSKSRC
                  ELSE 
!
              istat=0
      print *,' UMD lake mask opened OK   -- istat,MSKSRC:',istat,MSKSRC
      ...
      print *,' UMD lake mask rd err -- trying navy 10',istat
          ...
          print *,' ***** MSKSRC set to 0 MSKSRC=',MSKSRC
           ...
           print *,' navy10 lake mask rd fail - ios,MSKSRC:',ios,MSKSRC
              ...
              ELSE
      print *,' UMD lake mask read OK -- istat,MSKSRC:',istat,MSKSRC
...
!
     ...
     & kount1,kount2,ZAVG(1,1),ZAVG(imn,jmn),ZAVG(500,500)
...
!     &' orig:',i5,i3,' Lat=',f8.3,f9.3,'E')
            if(  ZAVG(i,j) .ne. zsave1 ) then 
          if ( i .le. 1201 .and. i .gt. 1200 )then
      write(6,152)i,j,ZAVG(i,j),ZSLM(i,j),zsave1,zsave2,ALAT,ALON,
     &      GICE(i,j)
          endif
            endif
...
C       SPECTRALLY TRUNCATE AND FILTER OROGRAPHY
        ...
        enddo
        print*,'filtering of Orography done',NR,NM,IM,JM
