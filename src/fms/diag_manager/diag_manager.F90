...
MODULE diag_manager_mod
  ...
  !   <OL>
  !     <LI> Create atm_diag_table as described in the
  !          <LINK SRC="atm_diag_table.html">atm_diag_table.F90</LINK>
  !          documentation.</LI>
  ...
  !          output.
  !          <B>NOTE:</B> ALL fields in atm_diag_table should be registered <I>BEFORE</I>
  ...
  !     <LI> Check for duplicate lines in atm_diag_table. </LI>
  !     <LI> <LINK SRC="atm_diag_table.html">atm_diag_table</LINK> can contain fields
  !          that are NOT written to any files. The file name in atm_diag_table of
  ...
  !          the the content of atm_diag_table is printed in stdout.</LI>
  !     <LI> New optional format of file information in <LINK SRC="atm_diag_table.html">atm_diag_table</LINK>.It is possible to have just
  ...
  USE time_manager_mod, ONLY: set_time, set_date, get_time, time_type, OPERATOR(>=), OPERATOR(>),&
       & OPERATOR(<), OPERATOR(==), OPERATOR(/=), OPERATOR(/), OPERATOR(+), print_time, print_date
  ...
  USE atm_diag_table_mod, ONLY: parse_atm_diag_table
  USE diag_output_mod, ONLY: set_diag_global_att!, get_diag_global_att
...
CONTAINS
  ...
  INTEGER FUNCTION register_diag_field_array(module_name, field_name, axes, init_time, &
       ...
       & do_not_log, err_msg, interp_method, tile_count, mask, red2reg, total_elements)
    CHARACTER(len=*), INTENT(in) :: module_name, field_name
    ...
    logical, optional, intent(in) :: red2reg
    integer, optional, intent(in) :: total_elements
    ...
    register_diag_field_array = register_static_field(module_name, field_name, axes,&
         ...
         mask=mask, red2reg=red2reg, total_elements=total_elements)
    ...
    IF ( register_diag_field_array < 0 ) THEN
       ! <ERROR STATUS="WARNING">
       !   module/output_field <modul_name>/<field_name> NOT found in atm_diag_table
       ! </ERROR>
       IF ( debug_diag_manager .OR. verbose1 ) THEN 
          IF ( mpp_pe() == mpp_root_pe() ) &
               & CALL error_mesg ('diag_manager_mod::register_diag_field', 'module/output_field '&
               &//TRIM(module_name)//'/'// TRIM(field_name)//' NOT found in atm_diag_table',&
               & WARNING) 
       END IF
    ELSE 
       ...
       DO j = 1, input_fields(field)%num_output_fields
          ...
          !output_fields(ind)%red2reg = .true.
          
          ! if (present(mask)) then
          !    allocate(output_fields(ind)%mask(size(mask,1),size(mask,2)))
          !    output_fields(ind)%mask = mask
          ! endif
  ...
  INTEGER FUNCTION register_static_field(module_name, field_name, axes, long_name, units,&
       ...
       & tile_count, mask, red2reg, total_elements)
    CHARACTER(len=*), INTENT(in) :: module_name, field_name
    ...
    ! Negative index returned if this field was not found in the atm_diag_table.
    ...
    DO j = 1, input_fields(field)%num_output_fields
       ...
       IF ( output_fields(out_num)%reduced_k_range ) THEN
          ...
          if (present(total_elements)) output_fields(out_num)%total_elements = total_elements
       ...
       if (present(mask)) then
          allocate(output_fields(out_num)%mask(size(mask,1),size(mask,2)))
          output_fields(out_num)%mask = mask
       endif
       
       output_fields(out_num)%red2reg=.false.
       if (present(red2reg)) output_fields(out_num)%red2reg=red2reg
  ...
  LOGICAL FUNCTION send_data_1d(diag_field_id, field, time, is_in, mask, rmask, ie_in, weight, err_msg)
    ...
    REAL, DIMENSION(SIZE(field(:)), 1, 1) :: weight_out
    ...
    IF ( PRESENT(mask) .OR. PRESENT(rmask) ) THEN
       IF ( PRESENT(is_in) .OR. PRESENT(ie_in) ) THEN
          send_data_1d = send_data_3d(diag_field_id, field_out, time, is_in=is_in, js_in=1, ks_in=1,&
               & mask=mask_out, ie_in=ie_in, je_in=1, ke_in=1, weight=weight_out, err_msg=err_msg)
       ELSE
          send_data_1d = send_data_3d(diag_field_id, field_out, time, mask=mask_out,&
               & weight=weight_out, err_msg=err_msg)
       END IF
    ELSE
       IF ( PRESENT(is_in) .OR. PRESENT(ie_in) ) THEN
          send_data_1d = send_data_3d(diag_field_id, field_out, time, is_in=is_in, js_in=1, ks_in=1,&
               & ie_in=ie_in, je_in=1, ke_in=1, weight=weight_out, err_msg=err_msg)
       ELSE
          send_data_1d = send_data_3d(diag_field_id, field_out, time, weight=weight_out, err_msg=err_msg)
       END IF
    END IF
  ...
  LOGICAL FUNCTION send_data_2d(diag_field_id, field, time, is_in, js_in, &
    ...
    REAL, DIMENSION(SIZE(field,1),SIZE(field,2),1) :: weight_out
    ...
    IF ( PRESENT(mask) .OR. PRESENT(rmask) ) THEN
       send_data_2d = send_data_3d(diag_field_id, field_out, time, is_in=is_in, js_in=js_in, ks_in=1, mask=mask_out,&
            & ie_in=ie_in, je_in=je_in, ke_in=1, weight=weight_out, err_msg=err_msg)
    ELSE
  ...
  LOGICAL FUNCTION send_data_3d(diag_field_id, field, time, is_in, js_in, ks_in, &
    ...
    REAL :: weight1
    REAL :: weight2(size(field,1),size(field,2),size(field,3))
    REAL :: missvalue
    INTEGER :: ksr, ker
    ...
    !write(*,'(A,9(I5))') 'is, ie, hi, n1, js, je, hj, n2, n3 =', is, ie, hi, n1, js, je, hj, n2, n3
                               ...
                               DO i=is, ie
                                  IF ( mask(i-is+1+hi, j-js+1+hj, k) ) THEN
                                     output_fields(out_num)%buffer(i-hi,j-hj,k1,sample) =&
                                          & output_fields(out_num)%buffer(i-hi,j-hj,k1,sample) +&
                                          & field(i-is+1+hi, j-js+1+hj, k) * weight2(i-is+1+hi, j-js+1+hj, k)  
                                     output_fields(out_num)%counter(i-hi,j-hj,k1,sample) =&
                                          & output_fields(out_num)%counter(i-hi,j-hj,k1,sample) + weight2(i-is+1+hi, j-js+1+hj, k)
                                  END IF
                               END DO
                            END DO
                         END DO
                               ...
                               DO i=is, ie
                                  IF ( mask(i-is+1+hi, j-js+1+hj, k) ) THEN
                                     ...
                                     output_fields(out_num)%counter(i-hi,j-hj,k,sample) =&
                                          &output_fields(out_num)%counter(i-hi,j-hj,k,sample) + weight2(i-is+1+hi,j-js+1+hj,k)
                                  END IF
                               END DO
                            END DO
                         END DO
...
!$OMP CRITICAL
                         DO k=ks, ke
                            DO j=js, je
                               DO i=is, ie
                                  !write(*,'(A,8(I4))') 'i-hi, j-hj, i-is+1+hi, j-js+1+hj =', i-hi, j-hj, i-is+1+hi, j-js+1+hj
                                  IF ( field(i-is+1+hi,j-js+1+hj,k) /= missvalue )  THEN
                                     output_fields(out_num)%buffer(i-hi,j-hj,k,sample) =&
                                          & output_fields(out_num)%buffer(i-hi,j-hj,k,sample) +&
                                          & field(i-is+1+hi,j-js+1+hj,k) * weight1  
...
!$OMP END CRITICAL
  ...
  INTEGER FUNCTION writing_field(out_num, at_diag_end, error_string, time)
    ...
    IF ( (output_fields(out_num)%time_ops) .AND. (.NOT. mix_snapshot_average_fields) ) THEN
       ...
       CALL diag_data_out(file_num, out_num, output_fields(out_num)%buffer, middle_time, red2reg=output_fields(out_num)%red2reg)
    ELSE
       CALL diag_data_out(file_num, out_num, &
            & output_fields(out_num)%buffer, output_fields(out_num)%next_output, red2reg=output_fields(out_num)%red2reg)
    END IF
  ...
  !
  !     A warning will be issued here if a field in atm_diag_table is not registered
  !   </DESCRIPTION>
  !   <TEMPLATE>
  ...
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     Open and read atm_diag_table. Select fields and files for diagnostic output.
  !   </DESCRIPTION>
  ...
  SUBROUTINE diag_manager_init(diag_model_subset, err_msg)
    ...
    CALL parse_atm_diag_table(DIAG_SUBSET=diag_subset_output, ISTAT=mystat, ERR_MSG=err_msg_local)
    IF ( mystat /= 0 ) THEN
       IF ( fms_error_handler('diag_manager_mod::diag_manager_init',&
            & 'Error parsing atm_diag_table. '//TRIM(err_msg_local), err_msg) ) RETURN
    END IF
...
!     Each test has its own <TT>atm_diag_table</TT>, see the source of <TT>diag_manager.F90</TT> for the list of <TT>atm_diag_tables</TT>
!     for the unit tests.
!   </TESTPROGRAM>
!   <FUTURE>
...
! Each test is dependent on the atm_diag_table, and different atm_diag_tables
...
! The atm_diag_table for each test is included below.

!--------------------------------------------------------------------------------------------------
! atm_diag_table for test 1
