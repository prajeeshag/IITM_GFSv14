MODULE atm_diag_table_mod
  ! <CONTACT EMAIL="GFDL.Climate.Model.Info@noaa.gov">
  !   Seth Underwood
  ! </CONTACT>
  ! <OVERVIEW>
  !   <TT>atm_diag_table_mod</TT> is a set of subroutines use to parse out the data from a <TT>atm_diag_table</TT>.  This module
  !   will also setup the arrays required to store the information by counting the number of input fields, output files, and
  !   files.
  ! </OVERVIEW>
  ! <DESCRIPTION>
  !   <TT>atm_diag_table_mod</TT> parses the <TT>atm_diag_table</TT> file, and sets up the required arrays to hold the information
  ...
  !   <TT>atm_diag_table</TT> file consists of comma-separated ASCII values.  The <TT>atm_diag_table</TT> essentially has three sections:
  ...
  !   Comments can be added to the <TT>atm_diag_table</TT> file by using the hash symbol (#) as the first character in the line.
  !
  !   All errors in the <TT>atm_diag_table</TT> will throw a <TT>FATAL</TT> error.  A simple utility <TT>atm_diag_table_chk</TT>has been
  !   added to the FRE tools suite to check a <TT>atm_diag_table</TT> for errors.  A brief usage statement can be obtained by running
  !   <TT>atm_diag_table_chk --help</TT>, and a man page like description can views by running <TT>perldoc atm_diag_table_chk</TT>.
  !
  !   Below is a description of the three sections.
  !   <OL>
  !     <LI>
  !       <B>Global Section:</B>  The first two lines of the <TT>atm_diag_table</TT> must contain the <I>title</I> and the <I>base
  ...
  !   </OL>
  !
  !   <H4><B>Sample <TT>atm_diag_table</TT></B></H4>
  !   <NL>
  !     <LI>
  !       <PRE>
  ! "diag manager test"
...
CONTAINS
  ...
  ! <SUBROUTINE NAME="parse_atm_diag_table">
  !   <OVERVIEW>
  !     Parse the <TT>atm_diag_table</TT> in preparation for diagnostic output.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     SUBROUTINE parse_atm_diag_table(diag_subset, istat, err_msg)
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     <TT>parse_atm_diag_table</TT> is the public interface to parse the atm_diag_table, and setup the arrays needed to store the
  !     requested diagnostics from the <TT>atm_diag_table</TT>.  <TT>parse_atm_diag_table</TT> will return a non-zero <TT>istat</TT> if
  !     a problem parsing the <TT>atm_diag_table</TT>.
  !
  !     NOT YET IMPLEMENTED: <TT>parse_atm_diag_table</TT> will parse through the <TT>atm_diag_table</TT> twice.  The first pass, will be
  ...
  !     Status of parsing the <TT>atm_diag_table</TT>.  A non-zero status indicates a problem parsing the table.
  !   </OUT>
  !   <OUT NAME="err_msg" TYPE="CHARACTER(len=*), OPTIONAL">
  !     Error message corresponding to the <TT>istat</TT> return value.
  !   </OUT>
  !   <ERROR STATUS="FATAL">
  !     atm_diag_table file does not exist.
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  ...
  !     Problem reading atm_diag_table, line numbers in errors may be incorrect.
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Problem reading the atm_diag_table (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Incorrect file description FORMAT in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Invalid file FORMAT for file description in the atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Invalid time axis units in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Invalid output frequency units in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Invalid NEW file frequency units in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Invalid file duration units in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Invalid start time in the file description in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Field description FORMAT is incorrect in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Packing is out of range for the field description in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  !   <ERROR STATUS="FATAL">
  !     Error in regional output description for field description in atm_diag_table. (line: <line_number>)
  !   </ERROR>
  SUBROUTINE parse_atm_diag_table(diag_subset, istat, err_msg)
    ...
    INTEGER :: record_len !< String length of the atm_diag_table line read in.
    INTEGER :: num_lines !< Number of lines in atm_diag_table
    ...
    CHARACTER(len=256) :: record_line !< Current line from the atm_diag_table.
    CHARACTER(len=256) :: local_err_msg !< Sting to hold local error messages.
    CHARACTER(len=DT_LINE_LENGTH), DIMENSION(:), ALLOCATABLE :: atm_diag_table
    ...
    stdlog_unit = stdlog()
    num_lines = get_ascii_file_num_lines('atm_diag_table', DT_LINE_LENGTH)
    allocate(atm_diag_table(num_lines))

    call read_ascii_file('atm_diag_table', DT_LINE_LENGTH, atm_diag_table)
    
    ! Read in the global file labeling string
    READ (UNIT=atm_diag_table(1), FMT=*, IOSTAT=mystat) global_descriptor
    IF ( mystat /= 0 ) THEN
       pstat = mystat
       IF ( fms_error_handler('atm_diag_table_mod::parse_atm_diag_table', 'Error reading the global descriptor from the diagnostic table.',&
            & err_msg) ) RETURN
    END IF
    
    ! Read in the base date
    READ (UNIT=atm_diag_table(2), FMT=*, IOSTAT=mystat) base_year, base_month, base_day, base_hour, base_minute, base_second
    IF ( mystat /= 0 ) THEN
       pstat = mystat
    ...
    IF ( get_calendar_type() /= NO_CALENDAR ) THEN
       IF ( base_year==0 .OR. base_month==0 .OR. base_day==0 ) THEN
          pstat = 101
          IF ( fms_error_handler('atm_diag_table_mod::parse_atm_diag_table', 'The base_year/month/day can not equal zero', err_msg) ) RETURN
       END IF
    ...
    parser: DO line_num=3, num_lines
       ...
       READ (atm_diag_table(line_num), FMT='(A)', IOSTAT=mystat) record_line
       ! Increase line counter, and put in string for use in warning/error messages.
       WRITE (line_number, '(I5)') line_num

       IF ( mystat > 0 ) THEN
          IF ( mpp_pe() == mpp_root_pe() ) &
               & CALL error_mesg("atm_diag_table_mod::parse_atm_diag_table",&
               & "Problem reading the atm_diag_table (line:" //line_number//").", FATAL)
          CYCLE parser
    ...
    ! Close the atm_diag_table file.
    deallocate(atm_diag_table)

    ! check duplicate output_fields in the atm_diag_table
    CALL check_duplicate_output_fields(ERR_MSG=local_err_msg)
    IF ( local_err_msg /= '' ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_atm_diag_table', TRIM(local_err_msg), err_msg) ) RETURN
    END IF

  END SUBROUTINE parse_atm_diag_table
  ! </SUBROUTINE>

  ! <PRIVATE>
  ! <SUBROUTINE NAME="open_atm_diag_table">
  !   <OVERVIEW>
  !     Open the atm_diag_table file.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     SUBROUTINE open_atm_diag_table(iunit, iostat)
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     Open the <TT>atm_diag_table</TT> file, and return the Fortran file unit number.
  !   </DESCRIPTION>
  !   <OUT NAME="iunit" TYPE="INTEGER">Fortran file unit number of the <TT>atm_diag_table</TT>.</OUT>
  !   <IN NAME="iostat" TYPE="INTEGER, OPTIONAL">
  !     Status of opening file.  If iostat == 0, file exists.  If iostat > 0, the atm_diag_table file does not exist.
  !   </IN>
  !   <OUT NAME="err_msg" TYPE="CHARACTER(len=*), OPTIONAL">
  !     String to hold the return error message.
  !   </OUT>
  SUBROUTINE open_atm_diag_table(iunit, iostat, err_msg)
    ...
    IF ( .NOT.file_exist('atm_diag_table') ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::open_atm_diag_table',&
            & 'atm_diag_table file does not exist.', err_msg) ) RETURN
    ELSE 
       pstat = 0
    END IF

    CALL mpp_open(iunit, 'atm_diag_table', action=MPP_RDONLY)
  END SUBROUTINE open_atm_diag_table
  ! </SUBROUTINE>
  ! </PRIVATE>

  ! <PRIVATE>
  ! <SUBROUTINE NAME="close_atm_diag_table">
  !   <OVERVIEW>
  !     Close the atm_diag_table file.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     SUBROUTINE close_atm_diag_table(iunit)
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     Closes the atm_diag_table file.
  !   </DESCRIPTION>
  !   <IN NAME="iunit" TYPE="INTEGER">Fortran file unit number of the <TT>atm_diag_table</TT>.</IN>
  SUBROUTINE close_atm_diag_table(iunit)
    INTEGER, INTENT(in) :: iunit

    CALL close_file(iunit)
  END SUBROUTINE close_atm_diag_table
  ! </SUBROUTINE>
  ! </PRIVATE>
  ...
  !   <OVERVIEW>
  !     Parse a file description line from the <TT>atm_diag_table</TT> file.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     TYPE(file_description_type) FUNCTION parse_file_line(line, istat, err_msg)
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     <TT>parse_file_line</TT> parses a file description line from the <TT>atm_diag_table</TT> file, and returns a
  ...
  !   </DESCRIPTION>
  !   <IN NAME="line" TYPE="CHARACTER(len=*)">Line to parse from the <TT>atm_diag_table</TT> file.</IN>
  ...
  TYPE(file_description_type) FUNCTION parse_file_line(line, istat, err_msg)
    ...
    IF ( mystat > 0 ) THEN
       pstat = mystat
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', 'Incorrect file description format in atm_diag_table.', err_msg) )&
            & RETURN
    END IF
    ...
    IF ( SCAN(parse_file_line%file_name, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in file_name in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_file_line%output_freq_units, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in output_freq_units in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_file_line%time_units, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in time_units in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_file_line%long_name, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in long_name in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_file_line%new_file_freq_units, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in new_file_freq_units in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_file_line%start_time_s, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in start_time_s in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_file_line%file_duration_units, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
            & 'Unallowed character in file_duration_units in the atm_diag_table.', err_msg) ) RETURN
    END IF
    ...
    IF ( parse_file_line%file_format > 2 .OR. parse_file_line%file_format < 1 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', 'Invalid file format for file description in the atm_diag_table.',&
            & err_msg) ) RETURN
    END IF
    ...
    IF ( parse_file_line%iTime_units < 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', 'Invalid time axis units in atm_diag_table.', err_msg) )&
            & RETURN
    END IF
    IF ( parse_file_line%iOutput_freq_units < 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', 'Invalid output frequency units in atm_diag_table.', err_msg) )&
            & RETURN
    END IF
    IF ( parse_file_line%iNew_file_freq_units < 0 .AND. parse_file_line%new_file_freq > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', 'Invalid new file frequency units in atm_diag_table.', err_msg) )&
            & RETURN
    END IF
    IF ( parse_file_line%iFile_duration_units < 0 .AND. parse_file_line%file_duration > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', 'Invalid file duration units in atm_diag_table.', err_msg) )&
            & RETURN
    END IF
    ...
    new_file_freq_present: IF ( parse_file_line%new_file_freq > 0 ) THEN ! New file frequency present.
       IF ( LEN_TRIM(parse_file_line%start_time_s) > 0 ) THEN ! start time present
          ...
          IF ( mystat /= 0 ) THEN 
             pstat = 1
             IF ( fms_error_handler('atm_diag_table_mod::parse_file_line',&
                  & 'Invalid start time in the file description in atm_diag_table.', err_msg) ) RETURN
          END IF
          parse_file_line%start_time = set_date(year, month, day, hour, minute, second, err_msg=local_err_msg)
          IF ( local_err_msg /= '' ) THEN
             pstat = 1
             IF ( fms_error_handler('atm_diag_table_mod::parse_file_line', local_err_msg, err_msg) ) RETURN
          END IF
  ...
  !   <OVERVIEW>
  !     Parse a field description line from the <TT>atm_diag_table</TT> file.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     TYPE(field_description_type) FUNCTION parse_field_line(line, istat, err_msg)
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     <TT>parse_field_line</TT> parses a field description line from the <TT>atm_diag_table</TT> file, and returns a
  ...
  TYPE(field_description_type) FUNCTION parse_field_line(line, istat, err_msg)
    ...
    IF ( mystat /= 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Field description format is incorrect in atm_diag_table.', err_msg) ) RETURN
    END IF
    ...
    IF ( SCAN(parse_field_line%module_name, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in module_name in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_field_line%field_name, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in field_name in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_field_line%output_name, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in output_name in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_field_line%file_name, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in file_name in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_field_line%time_sampling, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in time_sampling in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_field_line%time_method, UNALLOWED_ALL) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in time_method in the atm_diag_table.', err_msg) ) RETURN
    END IF
    IF ( SCAN(parse_field_line%spatial_ops, UNALLOWED_QTE) > 0 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Unallowed Unallowed character in spatial_ops in the atm_diag_table.', err_msg) ) RETURN
    END IF
    ...
    IF ( parse_field_line%pack > 8 .OR. parse_field_line%pack < 1 ) THEN
       pstat = 1
       IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
            & 'Packing is out of range for the field description in atm_diag_table.', err_msg) ) RETURN
    END IF
    ...
    IF ( lowercase(TRIM(parse_field_line%spatial_ops)) /= 'none' ) THEN
       ...
       IF ( mystat /= 0 ) THEN
          IF ( fms_error_handler('atm_diag_table_mod::parse_field_line',&
               & 'Error in regional output description for field description in atm_diag_table.', err_msg) ) RETURN
       END IF
    END IF
  ...
  !   <OVERVIEW>
  !     Determines if a line from the atm_diag_table file is a file.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     PURE LOGICAL FUNCTION is_a_file(line)
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     <TT>is_a_file</TT> checks a atm_diag_table line to determine if the line describes a file.  If the line describes a file, the
  !     <TT>is_a_file</TT> will return <TT>.TRUE.</TT>.  Otherwise, it will return <TT>.FALSE.</TT>
  !   </DESCRIPTION>
  !   <IN NAME="line" TYPE="CARACTER(len=*)">String containing the <TT>atm_diag_table</TT> line.</IN>
  PURE LOGICAL FUNCTION is_a_file(line)
    CHARACTER(len=*), INTENT(in) :: line
  ...
  !   </DESCRIPTION>
  !   <IN NAME="file_name_string" TYPE="CHARACTER(len=*)">String containing the file name from the <TT>atm_diag_table</TT>.</IN>
  PURE CHARACTER(len=128) FUNCTION fix_file_name(file_name_string)
  ...
  !   <OVERVIEW>
  !     Allocate the file, in and out field arrays after reading the <TT>atm_diag_table</TT> file.
  !   </OVERVIEW>
  !   <TEMPLATE>
  !     SUBROUTINE initialize_output_arrays()
  !   </TEMPLATE>
  !   <DESCRIPTION>
  !     After reading in the <TT>atm_diag_table</TT> file, the arrays that will hold the file, in, and out field data need to be
