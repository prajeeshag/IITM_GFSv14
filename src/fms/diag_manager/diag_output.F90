MODULE diag_output_mod
  ...
  PUBLIC :: diag_output_init, write_axis_meta_data, write_field_meta_data, done_meta_data,&
       & diag_field_out, diag_flush, diag_fieldtype, set_diag_global_att !, get_diag_global_att
...
!    CALL get_diag_global_att(gAtt)
    CALL mpp_write_meta(file_unit, 'grid_type', cval=TRIM(diag_global_att%grid_type))
    CALL mpp_write_meta(file_unit, 'grid_tile', cval=TRIM(diag_global_att%tile_name))
    if (allocated(diag_global_att%decomp)) CALL mpp_write_meta(file_unit, 'decomp', ival=diag_global_att%decomp(:))
  ...
  SUBROUTINE set_diag_global_att(component, gridType, tileName, decomp)
    CHARACTER(len=*),INTENT(in), optional :: component, gridType, tileName 
    ...
    if(present(component)) component_tmp = component
    ! Don't know how to set these for specific component
    ! Want to be able to say 
    ! if(output_file has component) then
    if(present(gridType)) diag_global_att%grid_type = gridType
    if(present(tileName)) diag_global_att%tile_name = tileName
    if (present(decomp)) then
       allocate(diag_global_att%decomp(size(decomp)))
       diag_global_att%decomp=decomp
    endif
    ! endif
