MODULE diag_util_mod
...
#ifndef notInterp
  use interpred_mod, only : interp2regular_fms
#endif
  ...
  PUBLIC get_subfield_size, log_diag_field_info, update_bounds, check_out_of_bounds,&
       ...
       & check_duplicate_output_fields, get_date_dif, get_subfield_vert_size, sync_file_times, get_time_string
...
11  FORMAT(a, ' since ', i4.4, '-', i2.2, '-', i2.2, ' ', i2.2, ':', i2.2, ':', i2.2)
  ...
  SUBROUTINE init_output_field(module_name, field_name, output_name, output_file,&
    ...
    ELSE
       ...
       IF ( file_num < 0 ) THEN
          ! <ERROR STATUS="FATAL">
          !   file <file_name> is NOT found in the atm_diag_table.
          ! </ERROR>
          CALL error_mesg('diag_util_mod::init_output_field', 'file '&
               & //TRIM(output_file)//' is NOT found in the atm_diag_table', FATAL)
       END IF
       IF ( tile_count > 1 ) THEN
    ...
    ! <ERROR STATUS="FATAL">
    !   improper time method in atm_diag_table for output field <output_name>
    ! </ERROR>
    IF ( method_selected /= 1 ) CALL error_mesg('diag_util_mod::init_output_field',&
         &'improper time method in atm_diag_table for output field:'//TRIM(output_name),FATAL)
...
11  FORMAT(A, ' since ', I4.4, '-', I2.2, '-', I2.2, ' ', I2.2, ':', I2.2, ':', I2.2)
  ...
  SUBROUTINE diag_data_out(file, field, dat, time, final_call_in, static_write_in, red2reg)
    INTEGER, INTENT(in) :: file, field
    REAL, DIMENSION(:,:,:,:), INTENT(inout) :: dat
    TYPE(time_type), INTENT(in) :: time
    LOGICAL, OPTIONAL, INTENT(in):: final_call_in, static_write_in, red2reg

    LOGICAL :: final_call, do_write, static_write
    INTEGER :: i, num, n, k
    REAL :: dif, time_data(2, 1, 1, 1), dt_time(1, 1, 1, 1), start_dif, end_dif
    REAL :: dat1(size(dat,1),size(dat,2),size(dat,3),size(dat,4))
    ...
    if (present(red2reg).and.red2reg) then
       if (allocated(output_fields(field)%mask)) then
          do n=1,size(dat,4)
             do k=1,size(dat,3)
#ifndef notInterp
                call interp2regular_fms(dat(:,:,k,n), dat1(:,:,k,n), output_fields(field)%mask)
#endif
             enddo
          enddo
       else
          do n=1,size(dat,4)
             do k=1,size(dat,3)
#ifndef notInterp
                call interp2regular_fms(dat(:,:,k,n), dat1(:,:,k,n))
#endif
             enddo
          enddo
       endif    
       CALL diag_field_out(files(file)%file_unit, output_fields(field)%f_type, dat1, dif)
    else
  ...
  SUBROUTINE write_static(file)
    ...
    DO j = 1, files(file)%num_fields
       ...
       CALL diag_data_out(file, i, output_fields(i)%buffer, files(file)%last_flush, .TRUE., .TRUE., red2reg=output_fields(i)%red2reg)
    END DO
    ! Close up this file   
  ...
  SUBROUTINE check_duplicate_output_fields(err_msg)
    ...
    IF ( err_msg_local /= '' ) THEN
       IF ( fms_error_handler(' ERROR in atm_diag_table',err_msg_local,err_msg) ) RETURN
    END IF
